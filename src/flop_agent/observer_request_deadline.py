...
can pin the room worker until retained history has already moved past its cursor.

This overlay adds a total wall-clock bound around the existing read-only live and
export functions. The hot lobby keeps the tight bound used by its local capture
shock absorber. The lower-volume core ``events`` room gets a longer export-only
wall-clock allowance because Production proved its retained-ring export can keep
streaming beyond 20 seconds without tripping HTTPX's inactivity timeout; startup
catch-up must be allowed to finish instead of remaining permanently fail-closed.

It performs no writes, signing, URL following, command execution, or secret
access.
"""
...
EVENTS_EXPORT_TOTAL_DEADLINE_SECONDS = 90.0
...
def _export_total_deadline(room: str) -> float:
    if room == "events":
        return EVENTS_EXPORT_TOTAL_DEADLINE_SECONDS
    return EXPORT_TOTAL_DEADLINE_SECONDS
...
async def read_room_export(client, room: str):
    try:
        return await asyncio.wait_for(
            ...
            timeout=_export_total_deadline(room),
        )
    except TimeoutError:
        return None, None, TOTAL_TIMEOUT_ERROR
