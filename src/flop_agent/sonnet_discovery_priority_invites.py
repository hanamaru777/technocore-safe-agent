"""One-shot fixed sonnet-2 discovery invitations for two priority writers.

This lane is intentionally separate from the already-completed three-invite state and
from the dormant Murphy lane. No text, room, target DID, URL, contest or request ID
is accepted from CLI input. Each invitation has a stable request ID. A durable attempt
marker precedes POST; an interrupted attempt can only reconcile and is never blindly
retried.
"""
...
NOTE_TEXT = (
    "Discovery invite from MinerMaru73: I am forming a sonnet-2 team and your writer "
    "registration is accepted. My own writer/team-room authoritative disposition is "
    "still being reconciled. This is a discovery invitation only, not roster consent "
    "or a word proposal. If you are currently available and interested, please reply "
    "in discovery."
)
INVITATIONS = (
    {
        "type": "sonnet.note.v1",
        "contest_id": "sonnet-2",
        "target_did": "did:key:z6MkqRmKyNcvzfYX2PRXj2TRoXMYDKDy6RQzL6VAwj4DBhhj",
    ...
    },
    {
        "type": "sonnet.note.v1",
        "contest_id": "sonnet-2",
        "target_did": "did:key:z6Mkt6jAezZ7WyPyN1bTXX63ps4vn8MSvNwtfA9JFgsqeFTB",
        "request_id": "maru-invite-fuego-20260914-1",
        "text": NOTE_TEXT,
    },
)
...
def state_path():
    return core.STATE / "signer" / "sonnet-2-discovery-priority-invites.json"
...
def validate(value: dict) -> dict:
    ...
    required = {
        "payload", "state", "nonce", "text_hash", "git_commit_sha",
    ...
    for request_id, entry in value["entries"].items():
        ...
        if entry["state"] not in {"new", "prepared", "attempting", "ambiguous", "posted"}:
            raise InviteError("invite_state_invalid")
        if entry["nonce"] is not None and (
            not isinstance(entry["nonce"], str)
            or not re.fullmatch(r"[0-9]{1,19}", entry["nonce"])
        ):
            raise InviteError("invite_state_invalid")
        if entry["state"] != "new" and entry["nonce"] is None:
            raise InviteError("invite_state_invalid")
        if entry["git_commit_sha"] is not None and not re.fullmatch(
            r"[0-9a-f]{40}", str(entry["git_commit_sha"])
        ):
            raise InviteError("invite_state_invalid")
        if entry["state"] in {"attempting", "ambiguous"} and not isinstance(
            entry["attempted_at"], str
        ):
            raise InviteError("invite_state_invalid")
...
def _post_one(state: dict, request_id: str) -> dict:
    ...
    signed = oracle_signer.with_vault_seed(
        lambda: core.invoke_signer("say", ROOM, entry["nonce"], text)
    )
    if len(signed) != 2 or signed[0] != DID:
        raise InviteError("did_mismatch")
    verify_signed_record(
        ROOM,
        {"from": DID, "nonce": entry["nonce"], "text": text, "sig": signed[1]},
    )
    require_identity()
    require_health()
    if not OPEN <= datetime.now(UTC) < CLOSE:
...
def _run_locked() -> dict:
    ...
    for item in INVITATIONS:
        ...
        entry = state["entries"][request_id]
        if entry["state"] == "posted":
            if not any(action["request_id"] == request_id for action in actions):
                actions.append(
                    {"request_id": request_id, "action": "already_posted", "seq": entry["seq"]}
                )
            continue
...
def main() -> None:
    if len(sys.argv) != 1:
        raise SystemExit("sonnet priority discovery invitations accept no arguments")
    try:
        result = run_once()
    except Exception:
