...
def _read_heartbeat_payload() -> dict | None:
    path = heartbeat_path()
    if not path.exists(): return None
    try: data = json.loads(path.read_text("utf-8"))
...
def _read_heartbeat_status() -> dict | None:
    data = _read_heartbeat_payload(); status = data.get("resident_status") if data else None
...
def load_state() -> dict:
    ...
    if not isinstance(state.get("daemon"), dict): state["daemon"] = defaults["daemon"]
    for key, value in defaults["daemon"].items(): state["daemon"].setdefault(key, value)
    ...
    if heartbeat is not None and observer.parse_time(heartbeat.get("last_refresh_at")):
        current = observer.parse_time(state.get("daemon", {}).get("last_refresh_at")); cached = observer.parse_time(heartbeat.get("last_refresh_at"))
...
def _status_from_state(state: dict, observed: dict | None = None) -> dict:
    ...
    candidates = state.get("candidates", {}).values(); pending = [item for item in candidates if isinstance(item, dict) and item.get("status") == "pending"]
    actionable = sum(item.get("priority") == "critical" or item.get("signals", {}).get("direct_public_signed") is True or (item.get("priority") == "high" and not item.get("signals")) for item in pending)
    return {"read_only": True, "uptime_started_at": state["daemon"]["started_at"], "health": cached.get("health", {}), "last_refresh_at": state["daemon"]["last_refresh_at"], "cursors": cached.get("cursors", {}), "message_gaps": cached.get("message_gaps", 0), "discovery_queue": cached.get("discovery_queue", 0), "agents_known": cached.get("agents_known", 0), "useful_candidates": len(pending), "critical_candidates": sum(item.get("priority") == "critical" for item in pending), "direct_candidates": sum(item.get("signals", {}).get("direct_public_signed") is True for item in pending), "actionable_candidates": actionable, "noise_ignored": state.get("metrics", {}).get("noise_ignored", 0), "returning_agents": cached.get("returning_agents", 0), "inbound": cached.get("inbound", 0), "approved": sum(isinstance(item, dict) and item.get("status") == "approved" for item in state.get("candidates", {}).values()), "rejected": sum(isinstance(item, dict) and item.get("status") == "rejected" for item in state.get("candidates", {}).values()), "expired": sum(isinstance(item, dict) and item.get("status") == "expired" for item in state.get("candidates", {}).values()), "published": len(state.get("published", [])), "paused": state["control"]["paused"], "discord_status": "not_connected"}


def _next_pending_expiry(state: dict) -> str | None:
    values = [observer.parse_time(item.get("expires_at")) for item in state.get("candidates", {}).values() if isinstance(item, dict) and item.get("status") == "pending"]
    stamp = min((value for value in values if value), default=None)
    return stamp.isoformat() if stamp else None


def write_heartbeat(state: dict | None = None, status: str = "ok", *, snapshot: dict | None = None, observer_revision: str | None = None, next_candidate_expiry: str | None = None) -> None:
    ...
    previous = _read_heartbeat_payload() or {}
    if observer_revision is None and state is not None: observer_revision = state.get("daemon", {}).get("observer_revision")
    if observer_revision is None: observer_revision = previous.get("observer_revision")
...
def save_state(state: dict) -> None:
    override = _control_override()
    if override is not None: state["control"]["paused"] = override
    state["updated_at"] = now(); observer.atomic_json_write(state_path(), state); write_heartbeat(state)
...
def quality(agent: dict, all_agents: list[dict], config: dict, frequencies: dict[str, int] | None = None) -> dict:
    ...
    norm = [normalized(text) for text in messages if normalized(text)]; duplicate = len(norm) - len(set(norm)); template = sum(any(re.search(pattern, text, re.I) for pattern in config["generic_templates"]) for text in messages); frequencies = frequencies or message_frequencies(all_agents); cluster = sum(max(0, frequencies.get(value, 0) - 1) for value in set(norm)); garbled_count = sum(garbled(text) for text in messages); specific = sum(bool(CONCRETE_RE.search(text)) for text in messages); poetic = sum(bool(POETIC_RE.search(text)) for text in messages); technical = [word for word in ("api", "python", "test", "protocol", "commit", "bug", "error") if any(re.search(rf"\b{word}\b", text, re.I) for text in messages)]; continuity = bool(agent["inferences"].get("repeat_seen")); facts = agent["facts"]; noise = min(1.0, (duplicate + template + poetic + garbled_count + cluster * 0.5) / max(1, len(messages))); score = 0.30 + min(0.08, 0.02 * max(0, len(facts.get("rooms", [])) - 1)) + (0.15 if continuity else 0) + min(0.25, 0.09 * specific) - min(0.60, 0.12 * (duplicate + template + poetic + garbled_count) + 0.04 * cluster)
...
def relationship(state: dict, agent: dict, assessment: dict) -> dict:
    ...
    messages = [item.get("text", "")[:280] for item in agent["facts"].get("recent_messages", []) if item.get("text")]; record["last_seen"] = agent["facts"]["last_seen"]; record["rooms"] = agent["facts"]["rooms"]; record["important_messages"] = messages[-20:]; record["topics"] = assessment["technical_depth_indicators"]; record["questions"] = [text for text in messages if "?" in text][-20:]; record["help_requests"] = [text for text in messages if re.search(r"\b(help|assist|support)\b", text, re.I)][-20:]; record["role_candidates"] = agent["inferences"].get("role_candidates", []); record["contribution_candidates"] = agent["inferences"].get("contribution_url_candidates", [])
...
def expire_candidates(state: dict, current: datetime | None = None) -> bool:
    ...
    for item in state["candidates"].values():
        ...
        if expiry and expiry <= current: item["status"] = "expired"; item["expired_at"] = now(); item["expiration_reason"] = "candidate_ttl_elapsed"; changed = True
    return changed
...
def _latest_candidate_times(state: dict) -> dict[str, datetime]:
    ...
    for item in state.get("candidates", {}).values():
        ...
        stamps = [observer.parse_time(item.get(key)) for key in ("published_at", "feedback_at", "created_at")]; stamp = max((value for value in stamps if value), default=None)
...
def cooldown_active(state: dict, did: str, current: datetime, seconds: int, latest_by_did: dict[str, datetime] | None = None) -> bool:
    latest = (latest_by_did or _latest_candidate_times(state)).get(did); return bool(latest and (current - latest).total_seconds() < seconds)
...
def _prune_terminal_candidates(state: dict, limit: int) -> bool:
    ...
    expired.sort(key=lambda pair: observer.parse_time(pair[1].get("expired_at") or pair[1].get("created_at")) or datetime.min.replace(tzinfo=UTC), reverse=True); protected = {record.get("candidate_id") for record in state.get("feedback", []) if isinstance(record, dict)} | {record.get("candidate_id") for record in state.get("published", []) if isinstance(record, dict)}; changed = False; keep = {key for key, _ in expired[:limit]} | {str(key) for key in protected if key}
...
def _prune_relationships(state: dict, observed: dict, limit: int) -> bool:
    ...
    current_fingerprints = set(observed.get("agents", {})); candidate_fingerprints = {item.get("fingerprint") for item in state.get("candidates", {}).values() if isinstance(item, dict) and item.get("status") in {"pending", "approved", "published"} and item.get("fingerprint")}; protected = current_fingerprints | candidate_fingerprints
    for fingerprint, record in relationships.items():
        if isinstance(record, dict) and (record.get("approval_rejection_history") or record.get("relationship_state") == "contacted" or record.get("our_previous_action") in {"approved", "rejected"}): protected.add(fingerprint)
    removable = [(fingerprint, record) for fingerprint, record in relationships.items() if fingerprint not in protected]; removable.sort(key=lambda pair: (observer.parse_time(pair[1].get("last_seen")) if isinstance(pair[1], dict) else None) or datetime.min.replace(tzinfo=UTC)); excess = max(0, len(relationships) - limit); changed = False
    for fingerprint, _ in removable[:excess]: del relationships[fingerprint]; changed = True
    return changed


def _observer_revision(observed: dict, config: dict) -> str:
    metrics = observed.get("metrics", {})
    payload = {"cursors": sorted((str(key), int(value)) for key, value in observed.get("cursors", {}).items() if isinstance(value, int)), "health": observed.get("health", {}).get("current"), "counts": [len(observed.get("agents", {})), len(observed.get("opportunities", [])), len(observed.get("discovery_queue", [])), len(observed.get("discovered_rooms", {}))], "metrics": [int(metrics.get("message_gaps", 0)), int(metrics.get("unique_returning_dids", 0)), int(metrics.get("inbound_mailbox_messages", 0))], "config": [config["candidate_cooldown_seconds"], config["candidate_ttl_seconds"], config["quality_threshold"], config["generic_templates"]]}
...
def _unchanged_observer_refresh(observed: dict, config: dict) -> dict | None:
    """Skip 5k-Agent rescoring when the shared Observer facts did not change."""
    if _control_override() is True: return None
    payload = _read_heartbeat_payload(); status = _heartbeat_status()
    if payload is None or status is None or status.get("paused") is True: return None
    revision = _observer_revision(observed, config)
    if payload.get("observer_revision") != revision: return None
    next_expiry = observer.parse_time(payload.get("next_candidate_expiry"))
    if next_expiry is not None and next_expiry <= datetime.now(UTC): return None
    status = dict(status); status["last_refresh_at"] = now(); write_heartbeat(snapshot=status, observer_revision=revision, next_candidate_expiry=payload.get("next_candidate_expiry")); return status


def refresh(observed_state: dict | None = None) -> dict:
    config = load_config(); fast_paused = _paused_heartbeat_refresh()
    ...
    observed = observed_state if observed_state is not None else observer.load_state(); own_did = observer.verified_did(); agents = [agent for agent in observed["agents"].values() if agent["did"] != own_did]; agents_by_did = {agent["did"]: agent for agent in agents}; frequencies = message_frequencies(agents); assessments = {agent["did"]: quality(agent, agents, config, frequencies) for agent in agents}; state["metrics"]["noise_ignored"] = sum(item["spam_noise_probability"] >= 0.45 for item in assessments.values()); state["cached_observer"] = {"health": observed["health"], "cursors": observed["cursors"], "message_gaps": observed["metrics"]["message_gaps"], "discovery_queue": len(observed["discovery_queue"]), "agents_known": len(observed["agents"]), "returning_agents": observed["metrics"]["unique_returning_dids"], "inbound": observed["metrics"]["inbound_mailbox_messages"]}
    ...
    for (did, room, seq), events in grouped.items():
        ...
        event = events[0]; candidate = {"candidate_id": candidate_id, "did": did, "fingerprint": agent["fingerprint"], "room": room, "seq": seq, "permalink": core.human_permalink(room, seq) if isinstance(seq, int) else None, "category": category, "priority": priority, "ranking_weight": float(state["learning"]["weights"].get(category, 1.0)), "why": f"{category} after quality filtering", "signals": assessment, "context": {"excerpt": event.get("text_excerpt", "")[:280], "untrusted": True}, "suggested_action": "review and optionally approve; approval does not post", "draft_reply": draft(agent, event, category), "created_at": now(), "expires_at": (current + timedelta(seconds=config["candidate_ttl_seconds"])).isoformat(), "status": "pending"}; state["candidates"][candidate_id] = candidate; latest_by_did[did] = current
        relationship_record = state["relationships"].get(agent["fingerprint"])
        if relationship_record: relationship_record["interaction_history"].append({"kind": "candidate_created", "candidate_id": candidate_id, "at": now()}); relationship_record["interaction_history"] = relationship_record["interaction_history"][-50:]
    for agent in agents:
        for message in agent["facts"].get("recent_messages", []):
            ...
            assessment = assessments[agent["did"]]; state["candidates"][candidate_id] = {"candidate_id": candidate_id, "did": agent["did"], "fingerprint": agent["fingerprint"], "room": message["room"], "seq": message["seq"], "permalink": core.human_permalink(message["room"], message["seq"]), "category": plan["category"], "priority": "medium", "ranking_weight": 1.0, "why": "signed public direct request mapped to an allowlisted topic", "signals": {"conversation_topic": plan["topic"], "direct_public_signed": True, "facts": {"inbound_to_us": False}}, "context": {"excerpt": message["text"][:280], "untrusted": True}, "suggested_action": "fixed-template reply only; no untrusted text is rendered", "draft_reply": "", "created_at": now(), "expires_at": (current + timedelta(seconds=config["candidate_ttl_seconds"])).isoformat(), "status": "pending", "safety_decision": plan["safety_decision"]}; latest_by_did[agent["did"]] = current
    _prune_terminal_candidates(state, config["max_expired_candidates"]); _prune_relationships(state, observed, config["max_relationships"]); override = _control_override()
    if override is True: state["control"]["paused"] = True
    state["daemon"]["observer_revision"] = _observer_revision(observed, config); state["daemon"]["last_refresh_at"] = now(); save_state(state); return _status_from_state(state, observed)
...
def list_candidates() -> dict:
    refresh(); rank = {"critical": 4, "high": 3, "medium": 2, "low": 1, "ignore": 0}; items = list(load_state()["candidates"].values()); return {"candidates": sorted(items, key=lambda item: (rank[item["priority"]] * item.get("ranking_weight", 1.0), item["created_at"]), reverse=True)}
...
def feedback(candidate_id: str, decision: str, reason: str | None = None) -> dict:
    ...
    item["status"] = decision; item["feedback_reason"] = reason; item["feedback_at"] = now(); record = {"candidate_id": candidate_id, "decision": decision, "reason": reason, "at": now()}; state["feedback"].append(record); rel = state["relationships"].get(item["fingerprint"])
    if rel: rel["approval_rejection_history"].append(record); rel["our_previous_action"] = decision; rel["last_interaction"] = now(); rel["relationship_state"] = "contacted" if decision == "approved" else rel["relationship_state"]; rel["interaction_history"].append({"kind": decision, "candidate_id": candidate_id, "at": now()}); rel["interaction_history"] = rel["interaction_history"][-50:]
    weights = state["learning"]["weights"]; old = float(weights.get(item["category"], 1.0)); delta = 0.05 if decision == "approved" else -0.05; weights[item["category"]] = max(0.5, min(1.5, round(old + delta, 3))); state["learning"]["history"].append({"category": item["category"], "old": old, "new": weights[item["category"]], "reason": reason, "at": now()}); save_state(state); return item
...
def publish_approved(candidate_id: str, confirm: bool) -> dict:
    ...
    did = core.current_did(); core.require_verified_did(did); record = core.post_signed(item["room"], item["draft_reply"], confirm, did=did, action="approved_candidate_publish"); state = load_state(); state["candidates"][candidate_id]["status"] = "published"; state["candidates"][candidate_id]["published_at"] = now(); state["published"].append({"candidate_id": candidate_id, "at": now(), "permalink": record["permalink"]}); save_state(state); return record


def export_state() -> str:
    allowed = [(core.STATE / "verified-did.json", "verified-did.json"), (observer.state_path(), "observer/observer-state.json"), (observer.config_path(), "observer/observer-config.json"), (state_path(), "observer/resident-state.json"), (config_path(), "observer/resident-config.json")]; export_dir = core.STATE / "resident-exports"; export_dir.mkdir(parents=True, exist_ok=True); stamp = datetime.now(UTC).strftime("%Y%m%dT%H%M%SZ"); archive = export_dir / f"resident-state-{stamp}.zip"; manifest = []
