"""Public, planning-only Sonnet-2 CLI.

This module is deliberately local-only: it reads three caller-supplied files and
uses the already-audited Sonnet planner/preflight kernel. It has no network,
signing, Technocore, X, shell, Vault, seed, or credential capability.
"""
...
# Public metadata checked against flop-labs/technocore-sonnet-challenge on
# 2026/09/13. These values are informational; mechanical dictionary integrity is
# independently enforced by sonnet_preflight.verify_cmudict().
CONTEST_METADATA = {
    "contest_id": "sonnet-2",
    "rules_version": "0.5",
...
def load_poem(path: str | Path) -> list[list[str]]:
    ...
    # Blank stanza separators are accepted as input convenience. Mechanical
    # canonicalization is still delegated to the existing preflight kernel.
