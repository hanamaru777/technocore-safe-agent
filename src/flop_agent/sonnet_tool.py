...
def _assignment_json(assignment: planner.AssignmentResult) -> list[list[dict[str, str]]]:
    return [[{"token": word.token, "contributor_did": word.contributor_did} for word in line] for line in assignment.lines]


def build_report(roster: list[str], lines: list[list[str]], dictionary: dict[str, int]) -> dict:
