...
def _default() -> dict: return {"schema_version": 2, "initialized": False, "seen": [], "next_poll_at": 0.0, "failures": 0}


def _load() -> dict:
    try: value = json.loads(state_path().read_text("utf-8"))
    except (FileNotFoundError, OSError, TypeError, ValueError): return _default()
    if not isinstance(value, dict) or value.get("schema_version") != 2 or not isinstance(value.get("seen"), list): return _default()
    return {"schema_version": 2, "initialized": value.get("initialized") is True, "seen": [x for x in value["seen"] if isinstance(x, str)][-SEEN_LIMIT:], "next_poll_at": float(value.get("next_poll_at", 0)), "failures": max(0, int(value.get("failures", 0)))}
...
def _remember(seen: list[str], event_id: str) -> None:
    if event_id not in seen: seen.append(event_id)
    if len(seen) > SEEN_LIMIT: del seen[:-SEEN_LIMIT]
...
def _public_evidence(rows: list[tuple[str, dict]]) -> list[tuple[str, dict, tuple[str, str]]]:
    ...
    for room, row in rows:
        ...
        if room == DISCOVERY_ROOM and payload.get("type") in {"sonnet.invite-response.v1", "sonnet.invite-status.v1"} and payload.get("contest_id") == "sonnet-2" and payload.get("request_id") in INVITE_IDS:
            result.append(("invite", row, ("MARU discovery invitation response", "署名済みの返信・状態を確認。roster consent や参加承諾は推測していません。")))
        if room == TEAM_ROOM and payload.get("contest_id") == "sonnet-2" and payload.get("game_id") == GAME_ID and payload.get("poem_room") == TEAM_ROOM and payload.get("type") in {"sonnet.team-setup-receipt.v1", "sonnet.team-generation-receipt.v1"} and isinstance(payload.get("referee_receipt"), str) and payload["referee_receipt"]:
...
def poll_notices(*, now: float | None = None, fetch=_fetch, room_read=_room_rows) -> list[str]:
    ...
    try:
        github = [(f"gh:{issue}", row, _github_relevant(issue, row)) for issue in ISSUES for row in fetch(issue)]
        public = [(room, row) for room in (DISCOVERY_ROOM, TEAM_ROOM) for row in room_read(room)]
    ...
    notices = []; seen = state["seen"]
    for source, row, relevant in evidence:
        event_id = _event_id(source, row); initial_old = not state["initialized"] and not _created_after(row, current)
        if relevant and event_id not in seen and not initial_old:
