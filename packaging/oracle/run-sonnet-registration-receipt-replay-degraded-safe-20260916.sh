...
REF=refs/remotes/origin/registration-receipt-replay-degraded-helper
PROD_HEAD=51d058b45a66259074eb8cb22dda49b4cb26aea8
BASE_BLOB=dc70eb9aa01f2c8efca6cfb9db0f9d2c19b6a8cf
ADAPTER_BLOB=53b8ff51d946a4b012ec0ae36269b95f4edae09c
BASE=/run/sonnet_registration_receipt_replay_20260916.py
ADAPTER=/run/sonnet_registration_receipt_replay_degraded_20260916.py
LAUNCHER=/run/sonnet_registration_receipt_replay_degraded_20260916_launcher.py
UNIT=/run/systemd/system/technocore-safe-agent-sonnet-registration-receipt-replay-degraded.service
UNIT_NAME=technocore-safe-agent-sonnet-registration-receipt-replay-degraded.service
...
cleanup() {
  rm -f "$UNIT" "$BASE" "$ADAPTER" "$LAUNCHER"
...
[[ "$("${GIT[@]}" rev-parse "$REF:src/flop_agent/sonnet_registration_receipt_replay_20260916.py")" == "$BASE_BLOB" ]] || fail base_blob_mismatch
[[ "$("${GIT[@]}" rev-parse "$REF:src/flop_agent/sonnet_registration_receipt_replay_degraded_20260916.py")" == "$ADAPTER_BLOB" ]] || fail adapter_blob_mismatch
...
[[ "$HEALTH" == ok || "$HEALTH" == degraded ]] || fail replay_health_not_allowed
...
"${GIT[@]}" show "$REF:src/flop_agent/sonnet_registration_receipt_replay_20260916.py" > "$BASE"
"${GIT[@]}" show "$REF:src/flop_agent/sonnet_registration_receipt_replay_degraded_20260916.py" > "$ADAPTER"
chmod 0444 "$BASE" "$ADAPTER"
cat > "$LAUNCHER" <<'PY'
...
load("flop_agent.sonnet_registration_receipt_replay_20260916", "/run/sonnet_registration_receipt_replay_20260916.py")
adapter = load("flop_agent.sonnet_registration_receipt_replay_degraded_20260916", "/run/sonnet_registration_receipt_replay_degraded_20260916.py")
adapter.main()
PY
chmod 0444 "$LAUNCHER"
...
cat > "$UNIT" <<'UNIT'
[Unit]
Description=One exact idempotent Sonnet-2 MARU registration receipt replay under fresh degraded-safe gate
...
ExecStart=/opt/technocore-safe-agent/.venv/bin/python /run/sonnet_registration_receipt_replay_degraded_20260916_launcher.py
...
ReadOnlyPaths=/run/sonnet_registration_receipt_replay_20260916.py /run/sonnet_registration_receipt_replay_degraded_20260916.py /run/sonnet_registration_receipt_replay_degraded_20260916_launcher.py
