...
echo '=== PROD438 CAMPAIGN READINESS ROLLOUT V2 ==='
echo "UTC=$(date -u '+%Y-%m-%dT%H:%M:%SZ')"

stop_pre() {
  echo "PROD438V2=STOP:$1"
...
for cmd in git systemctl stat runuser cut sort; do
...
service_snapshot() {
  local unit=$1
  local active sub pid nr result start_us age_s
...
capture_service() {
  ...
  local snap rc
  snap=$(service_snapshot "$unit")
  rc=$?
  if [[ $rc -eq 2 ]]; then
    stop_pre "${label,,}_not_stable_long_enough"
  elif [[ $rc -ne 0 ]]; then
    stop_pre "${label,,}_not_healthy"
  fi
  echo "PRE_SERVICE=$label SNAPSHOT=$snap"
  printf -v "$varname" '%s' "$snap"
}

echo '--- PRE SERVICE GATES ---'
capture_service RESIDENT "$RES" PRE_RES
capture_service CAPTURE "$CAP" PRE_CAP
capture_service SIGNER "$SIGN" PRE_SIGN
capture_service DISCORD "$DISC" PRE_DISC
...
verify_same_service() {
  ...
  local current
  current=$(service_snapshot "$unit") || stop_post "${label,,}_changed_or_unhealthy"
  echo "POST_SERVICE=$label SNAPSHOT=$current"
  [[ "$current" == "$expected" ]] || stop_post "${label,,}_identity_changed"
}

echo '--- POST SERVICE GATES ---'
verify_same_service RESIDENT "$RES" "$PRE_RES"
verify_same_service CAPTURE "$CAP" "$PRE_CAP"
verify_same_service SIGNER "$SIGN" "$PRE_SIGN"
verify_same_service DISCORD "$DISC" "$PRE_DISC"

META_ACTIVE=$(systemctl is-active "$META" 2>/dev/null || true)
META_RESULT=$(systemctl show "$META" -p Result --value 2>/dev/null || true)
echo "POST_METADATA_BLOCK ACTIVE=$META_ACTIVE RESULT=$META_RESULT"
...
echo 'FLOP_WRITE=NO'
echo 'X_WRITE=NO'
echo '=== PROD438_CAMPAIGN_READINESS_ROLLOUT_V2=PASS ==='
echo 'DO_NOT_RERUN=YES'
exit 0
