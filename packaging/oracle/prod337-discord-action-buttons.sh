...
common_gate() {
  ...
  echo "PROD337_GATE=$label observer=$health core=$events/$messages radar=$radar heartbeat_age=$age ledger_count=$ledger_count staging=$staging staged=$staged action_pending=$pending action_total=$total candidates=$candidates"
}
...
rollback() {
  ...
  if [[ $DONE -eq 0 && $CUTOVER_STARTED -eq 1 ]]; then
    ...
    echo "PROD337_ROLLBACK=COMPLETE code=$PRE discord_restored=$DISCORD_RESTARTED" >&2
  fi
  exit "$rc"
}
trap rollback EXIT
...
[[ $ACTUAL == "$EXPECTED" ]] || {
  echo "PROD337=STOP:changed_file_allowlist_mismatch" >&2
...
echo "PROD337_PREFLIGHT=PASS pre=$PRE target=$TARGET"
...
SMOKE_DIR=$(mktemp -d /tmp/prod337-action-ui.XXXXXX)
...
row=airdrop_approval.stage_request(
    action_class="x_post",
    payload_sha256="a"*64,
    source_event_id="prod337-local-smoke",
    summary="PROD337 isolated local-only UI smoke",
