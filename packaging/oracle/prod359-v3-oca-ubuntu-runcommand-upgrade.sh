#!/usr/bin/env bash
# Third one-shot guarded Oracle Cloud Agent snap upgrade for Ubuntu Run Command support.
# Built only after #360 and #362 were consumed and independently reconciled. Branch-only; DO NOT MERGE.
# PR #360 and PR #362 must never be rerun. Any terminal PASS/STOP/disconnect here => DO_NOT_RERUN until a new independent reconciliation.
set -Eeuo pipefail
umask 077
...
app_gate() {
  ...
  echo "PROD359V3_GATE=$label observer=$OBSERVER_HEALTH evidence_based_continuity=yes core=$EXPECTED_CORE_EVENTS/$EXPECTED_CORE_MESSAGES root_imds=200 technocore_imds_blocked=yes"
}
...
echo "PROD359V3_PREFLIGHT=PASS version=$PRE_ACTUAL_VERSION revision=$PRE_ACTUAL_REVISION tracking=$TRACKING hold=$HOLD stable=$REMOTE_STABLE/$REMOTE_STABLE_REVISION observer=$OBSERVER_HEALTH"
