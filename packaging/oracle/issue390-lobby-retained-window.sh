#!/usr/bin/env bash
# Issue #390: read-only Lobby lag / retained-window attribution.
set -u
umask 077
...
echo '=== ISSUE390 LOBBY RETAINED-WINDOW READ-ONLY ATTRIBUTION ==='
echo "UTC=$(date -u '+%Y-%m-%dT%H:%M:%SZ')"

if [[ $EUID -ne 0 ]]; then
  echo 'ISSUE390_LAG=STOP:not_root'
  exit 0
fi

for path in "$APP/.git" "$OBS" "$RESHB"; do
  if [[ ! -e "$path" ]]; then
    echo 'ISSUE390_LAG=STOP:required_path_missing'
    exit 0
  fi
done
...
if [[ "$APP_HEAD" != "$EXPECTED_APP_HEAD" || -n "$WORKTREE" ]]; then
  echo 'ISSUE390_LAG=STOP:repo_baseline_changed'
  exit 0
fi
...
do
  ...
  if [[ "$active" != active || "$pid" != "$expected_pid" || "$nr" != "$expected_nr" || "$result" != success ]]; then
    echo "ISSUE390_LAG=STOP:service_baseline_changed:$label"
    exit 0
  fi
done

python3 - "$OBS" "$RESHB" "$EXPECTED_CORE_EVENTS" "$EXPECTED_CORE_MESSAGES" <<'PY'
import json
import pathlib
import sys
...
expected_messages=int(sys.argv[4])
...
if (events,messages)!=(expected_events,expected_messages):
    print("ISSUE390_LAG_STATE=STOP:core_moved_again")
    raise SystemExit(20)
...
print(f"RESIDENT_HEARTBEAT_STATUS={hb.get('status','missing')}")
...
print(f"RESIDENT_HEARTBEAT_AGE={age(hb.get('updated_at')):.1f}")
...
PY
STATE_RC=$?
echo "ISSUE390_LAG_STATE_RC=$STATE_RC"
if [[ "$STATE_RC" -ne 0 ]]; then
  echo 'ISSUE390_LAG=STOP:state_failed'
  exit 0
fi
...
timeout=httpx.Timeout(8.0,connect=3.0,pool=3.0)

def valid_rows(payload):
...
with httpx.Client(timeout=timeout,follow_redirects=False) as client:
    ...
    try:
        with client.stream("GET",BASE+"/r/lobby/export") as response:
            ...
            for raw in response.iter_lines():
                ...
                if first is None:
                    first=seq
                last=seq
        print("EXPORT_VALID_ROW_COUNT="+str(export_count))
        print("EXPORT_INVALID_ROW_COUNT="+str(bad))
        ...
        if first is not None and first > cursor + 1:
...
echo 'DID_OUTPUT=NO'
echo '=== ISSUE390_LOBBY_LAG_ATTRIBUTION=COMPLETE_READ_ONLY ==='
