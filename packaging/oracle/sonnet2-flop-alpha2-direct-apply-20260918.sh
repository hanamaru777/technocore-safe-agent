...
REQ=maru-flop-alpha2-apply-20260918-1
SAFETY="$STATE/observer-safety.json"

stop() {
  trap - ERR
  echo "FLOP_ALPHA2=STOP:$1"
  echo "REQUEST_ID=$REQ"
...
echo "FLOP_ALPHA2_PREFLIGHT=PASS health=$HEALTH age=$AGE core=$EVENTS/$MESSAGES"

TMP=$(mktemp /tmp/maru-flop-alpha2.XXXXXX.py)
trap 'rm -f "$TMP"' EXIT
...
LEADER="did:key:z6MkgLmJkvJ1qGxBsyfrWRPsG1U1w3tzvKxRZP9TDRK37Ts4"
REF="did:key:z6MkowHQwsx9xr84WbWN3YCnKutyBnBXkT1ChKY4uEAAMzte"

DISC="mb-sonnet-2-discovery"
TEAM="d-sonnet-2-team-flop-alpha-2"
GAME="flop-alpha-2"
REQUEST_ID="maru-flop-alpha2-apply-20260918-1"
STATE=core.STATE/"signer"/"sonnet-2-flop-alpha2-apply-20260918.json"
...
PAYLOAD={
  ...
  "target_did":LEADER,
  "text":(
    "MARU direct non-binding application for flop-alpha-2. I saw your recent seat-open announcements for the referee-provisioned team. "
    "Fresh room authority still shows generation 2 setup accepted (resetup-flop-alpha-2-2 / intake 110123) and no accepted word in the team room. "
    "MARU DID is "+MARU+" and currently has zero live roster consent. I am NOT claiming an explicit public writer receipt; MARU's separate team-request was referee-accepted at discovery seq 148670 / intake 869093 while the missing writer-ledger receipt remains under operator lookup. "
    "If a seat is still genuinely open, please first run your exact poem/turn solver against MARU's DID letters and current roster state. "
    "Only if eligibility + solver fit both pass, send an exact current generation-2 MARU-inclusive roster proposal. This application is not roster consent."
  ),
}
...
def require_team_still_pre_word():
    ...
    setup_ok=any(
      ...
      and d.get("request_id")=="resetup-flop-alpha-2-2"
      and d.get("status")=="accepted" and d.get("room_generation")==2
      and d.get("intake_seq")==110123
      for r,d in rows
    )
    if not room_ok or not setup_ok:stop("authoritative_setup_changed")
    later=[r.get("seq") for r,_ in rows if isinstance(r.get("seq"),int) and r["seq"]>3]
    if later:
      print("FLOP_ALPHA2_TEAM_NEW_SEQS="+",".join(map(str,sorted(later))))
      stop("team_already_advanced")

def inspect_discovery():
    existing=None
    for r,d in verified(DISC,200):
        if d.get("request_id")==REQUEST_ID:
            if r.get("from")!=MARU:stop("request_id_collision")
            if d!=PAYLOAD or r.get("text")!=TEXT:stop("existing_payload_conflict")
            existing=r
        members=d.get("members") if isinstance(d.get("members"),list) else []
        if d.get("type")=="sonnet.roster.v1" and d.get("game_id")==GAME and MARU in members:
            print("FLOP_ALPHA2_MARU_ROSTER_ALREADY_PRESENT="+json.dumps({
              "seq":r.get("seq"),"ts":r.get("ts"),"sender":r.get("from"),
              "request_id":d.get("request_id"),"members":members,
              "room_generation":d.get("room_generation"),"poem_room":d.get("poem_room")
            },ensure_ascii=False,sort_keys=True))
            stop("maru_roster_already_present")
        if r.get("from")==LEADER:
            blob=json.dumps(d,ensure_ascii=False)
            if d.get("target_did")==MARU or MARU in blob:
                print("FLOP_ALPHA2_LEADER_REPLY_ALREADY_PRESENT="+json.dumps({
...
if STATE.exists():
    ...
    if prior.get("state")=="posted":
        print("FLOP_ALPHA2=ALREADY_POSTED seq="+str(prior.get("seq"))+" ts="+str(prior.get("ts")))
        print("REQUEST_ID="+REQUEST_ID)
        print("DO_NOT_RERUN_THIS_BLOCK=YES")
        raise SystemExit(0)
...
require_team_still_pre_word()
existing=inspect_discovery()
if existing is not None:
    persist("posted",seq=existing.get("seq"),ts=existing.get("ts"),reconciled=True)
    print("FLOP_ALPHA2=RECONCILED seq="+str(existing.get("seq"))+" ts="+str(existing.get("ts")))
    print("REQUEST_ID="+REQUEST_ID)
    print("DO_NOT_RERUN_THIS_BLOCK=YES")
    raise SystemExit(0)
...
if existing is not None:
    persist("posted",seq=existing.get("seq"),ts=existing.get("ts"),reconciled=True)
    print("FLOP_ALPHA2=RECONCILED seq="+str(existing.get("seq")))
    print("REQUEST_ID="+REQUEST_ID)
    print("DO_NOT_RERUN_THIS_BLOCK=YES")
    raise SystemExit(0)
...
try:
    ...
    print("FLOP_ALPHA2=STOP:submission_unknown")
    print("REQUEST_ID="+REQUEST_ID)
...
print("FLOP_ALPHA2=PASS seq="+str(row["seq"])+" ts="+row["ts"])
print("REQUEST_ID="+REQUEST_ID)
...
while True:
    for r,d in verified(DISC,200):
        seq=r.get("seq")
        if not isinstance(seq,int) or seq<=row["seq"]:continue
        key=(seq,r.get("from"))
        if key in seen:continue
        members=d.get("members") if isinstance(d.get("members"),list) else []
        relevant=(d.get("game_id")==GAME and (r.get("from")==LEADER or d.get("target_did")==MARU or MARU in members or MARU in json.dumps(d,ensure_ascii=False)))
        if not relevant:continue
        seen.add(key)
        print("FLOP_ALPHA2_FOLLOWUP="+json.dumps({
          "seq":seq,"ts":r.get("ts"),"sender":r.get("from"),"type":d.get("type"),
          "request_id":d.get("request_id"),"target_did":d.get("target_did"),
          "text":d.get("text"),"members":members if members else None,
          "room_generation":d.get("room_generation"),"poem_room":d.get("poem_room"),
          "status":d.get("status"),"reason":d.get("reason")
        },ensure_ascii=False,sort_keys=True))
    for r,d in verified(TEAM,100):
        if isinstance(r.get("seq"),int) and r["seq"]>3:
            key=("team",r["seq"])
            if key in seen:continue
            seen.add(key)
            print("FLOP_ALPHA2_TEAM_UPDATE="+json.dumps({
              "seq":r.get("seq"),"ts":r.get("ts"),"sender":r.get("from"),
              "type":d.get("type"),"request_id":d.get("request_id"),
              "status":d.get("status"),"reason":d.get("reason"),
              "word":d.get("word"),"version":d.get("version")
            },ensure_ascii=False,sort_keys=True))
    if time.monotonic()>=end:break
    time.sleep(10)

print("FLOP_ALPHA2_WATCH=COMPLETE_READ_ONLY")
PY
...
echo "FLOP_ALPHA2_HELPER=COMPLETE"
