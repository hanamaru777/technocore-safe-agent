...
REQ=maru-rescue-v3-plan-20260918-2
...
BASELINE_LAST=150247
RID="maru-rescue-v3-plan-20260918-2"
...
def inspect_current_tail():
    body,rows=get_rows(DISC,BASELINE_LAST)
    last=body.get("last_seq")
    if not isinstance(last,int) or last<BASELINE_LAST:
        stop("discovery_tail_unreadable:last="+str(last))
    if last-BASELINE_LAST>180:
        stop("discovery_tail_overflow:last="+str(last))
    existing=None
    for r,d in rows:
