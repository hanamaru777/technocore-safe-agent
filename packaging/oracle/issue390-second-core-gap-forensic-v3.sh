#!/usr/bin/env bash
# Issue #390: classify the second protected-core regression while allowing Lobby cursor progress.
set -u
umask 077
...
TMPDIR=$(mktemp -d /tmp/issue390-gapv3.XXXXXX)
cleanup() { rm -rf -- "$TMPDIR"; }
trap cleanup EXIT

echo '=== ISSUE390 SECOND CORE GAP READ-ONLY FORENSIC V3 ==='
echo "UTC=$(date -u '+%Y-%m-%dT%H:%M:%SZ')"

if [[ $EUID -ne 0 ]]; then
  echo 'ISSUE390_GAPV3=STOP:not_root'
  exit 0
fi

for path in "$APP/.git" "$APP_PY" "$OBS" "$OBHB" "$RESHB" "$OBSCFG"; do
  if [[ ! -e "$path" ]]; then
    echo 'ISSUE390_GAPV3=STOP:required_path_missing'
    exit 0
  fi
done
...
if [[ "$APP_HEAD" != "$EXPECTED_APP_HEAD" || -n "$WORKTREE" ]]; then
  echo 'ISSUE390_GAPV3=STOP:repo_baseline_changed'
  exit 0
fi
...
svc() {
  ...
  if [[ "$active" != active || "$pid" != "$expected_pid" || "$nr" != "$expected_restarts" || "$result" != success ]]; then
    echo "ISSUE390_GAPV3=STOP:service_baseline_changed:$label"
    exit 0
  fi
}
...
analyze_state() {
  local tag=$1
  "$APP_PY" - "$tag" "$OBS" "$OBHB" "$RESHB" "$OBSCFG" \
    "$EXPECTED_CORE_EVENTS" "$EXPECTED_CORE_MESSAGES" \
    "$PRIOR_CORE_EVENTS" "$PRIOR_CORE_MESSAGES" "$PRIOR_LOBBY_CURSOR" "$GAP_EVENT_CURSOR" <<'PY'
import hashlib
import json
import pathlib
import sys
...
tag=sys.argv[1]
OBS=pathlib.Path(sys.argv[2])
OBHB=pathlib.Path(sys.argv[3])
RESHB=pathlib.Path(sys.argv[4])
CFG=pathlib.Path(sys.argv[5])
EXPECTED_EVENTS=int(sys.argv[6])
EXPECTED_MESSAGES=int(sys.argv[7])
PRIOR_EVENTS=int(sys.argv[8])
PRIOR_MESSAGES=int(sys.argv[9])
PRIOR_CURSOR=int(sys.argv[10])
GAP_EVENT_CURSOR=int(sys.argv[11])
...
print(f"{tag}_CORE_EVENT_DELTA_FROM_PRIOR={events-PRIOR_EVENTS}")
print(f"{tag}_CORE_MESSAGE_DELTA_FROM_PRIOR={messages-PRIOR_MESSAGES}")
print(f"{tag}_LOBBY_CURSOR_DELTA_FROM_PRIOR={lobby_cursor-PRIOR_CURSOR}")
print(f"{tag}_LOBBY_CURSOR_ADVANCE_AFTER_GAP_EVENT={lobby_cursor-GAP_EVENT_CURSOR}")
print(f"{tag}_OBSERVER_UPDATED_AT={state.get('updated_at','missing')}")
print(f"{tag}_OBSERVER_HEALTH_CURRENT={(state.get('health') or {}).get('current','missing')}")
...
if events != EXPECTED_EVENTS or messages != EXPECTED_MESSAGES:
    print(f"{tag}_STATE=STOP:core_moved_again")
    raise SystemExit(20)
if lobby_cursor < GAP_EVENT_CURSOR:
    print(f"{tag}_STATE=STOP:lobby_cursor_regressed")
    raise SystemExit(21)
...
if tag == "T0":
    ...
    if isinstance(last,dict):
        ...
        core_delta=messages-PRIOR_MESSAGES
        if isinstance(mf,int) and isinstance(mt,int):
            length=mt-mf+1
            print(f"LAST_GAP_RANGE_LENGTH={length}")
            print("LAST_GAP_STARTS_AT_PRIOR_CURSOR_NEXT="+("YES" if mf == PRIOR_CURSOR+1 else "NO"))
            print("LAST_GAP_ENDS_AT_GAP_EVENT_CURSOR="+("YES" if mt == GAP_EVENT_CURSOR else "NO"))
            print("LAST_GAP_LENGTH_EQUALS_CORE_DELTA="+("YES" if length == core_delta else "NO"))
    else:
...
echo '--- STATE SNAPSHOT T0 ---'
T0_OUT=$(analyze_state T0)
T0_RC=$?
printf '%s\n' "$T0_OUT"
if [[ "$T0_RC" -eq 20 ]]; then
  echo 'ISSUE390_GAPV3=STOP:core_moved_again_at_t0'
  exit 0
fi
if [[ "$T0_RC" -eq 21 ]]; then
  echo 'ISSUE390_GAPV3=STOP:lobby_cursor_regressed_at_t0'
  exit 0
fi
if [[ "$T0_RC" -ne 0 ]]; then
  echo 'ISSUE390_GAPV3=STOP:state_forensic_failed_at_t0'
  exit 0
fi
...
proc_snapshot T0
sleep 10
echo '--- STATE SNAPSHOT T10 ---'
T10_OUT=$(analyze_state T10)
T10_RC=$?
printf '%s\n' "$T10_OUT"
if [[ "$T10_RC" -eq 20 ]]; then
  echo 'ISSUE390_GAPV3=STOP:core_moved_again_at_t10'
  exit 0
fi
if [[ "$T10_RC" -eq 21 ]]; then
  echo 'ISSUE390_GAPV3=STOP:lobby_cursor_regressed_at_t10'
  exit 0
fi
if [[ "$T10_RC" -ne 0 ]]; then
  echo 'ISSUE390_GAPV3=STOP:state_forensic_failed_at_t10'
  exit 0
fi
echo '--- CAPTURE ACTIVITY T10 ---'
proc_snapshot T10
...
echo '=== ISSUE390_SECOND_CORE_GAP_FORENSIC_V3=COMPLETE_READ_ONLY ==='
