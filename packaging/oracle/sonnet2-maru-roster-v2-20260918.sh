...
pre_stop() {
  trap - ERR
  echo "MARU_ROSTER_V2=STOP_PRE_SIGN:$1"
...
echo "MARU_ROSTER_V2_PREFLIGHT=PASS health=$HEALTH age=$AGE core=$EVENTS/$MESSAGES"

TMP=$(mktemp /tmp/maru-roster-v2.XXXXXX.py)
trap 'rm -f "$TMP"' EXIT
...
WEN="did:key:z6MkrMxcLD9FLxoNHCfPxRU7nkRGPAE9ePSU8ZMDq3VUSttk"

DISC="mb-sonnet-2-discovery"
TEAM="d-sonnet-2-team-maru-rescue-1"
GAME="maru-rescue-1"
GEN=1
BASELINE_LAST=149592
RID="maru-rescue-roster-20260918-2"
MEMBERS=[MARU,GRID,PUPH,WEN]
PAYLOAD={
...
def stop_terminal(reason,state="aborted_after_sign"):
    ...
    print("MARU_ROSTER_V2=STOP_TERMINAL:"+reason)
...
def reconcile_request():
    ...
    for r,d in rows:
        if isinstance(d,dict) and d.get("request_id")==RID:
            ...
            print("MARU_ROSTER_V2=RECONCILED seq="+str(r.get("seq")))
...
if STATE.exists():
    ...
    print("MARU_ROSTER_V2=STOP_PRIOR_STATE:"+str(prior.get("state")))
    print("REQUEST_ID="+RID)
    print("DO_NOT_RERUN=YES")
    raise SystemExit(1)
...
try:
    ...
    print("MARU_ROSTER_V2=STOP:submission_unknown")
    print("REQUEST_ID="+RID)
...
print("MARU_ROSTER_V2_POST=PASS seq="+str(row["seq"])+" request_id="+RID)
    ...
    for rr,d in rows:
        ...
        if isinstance(d,dict):
            if d.get("type")=="sonnet.receipt.v1" and d.get("request_id")==RID and rr.get("from")==REF:
                receipt=(seq,d)
                print("MARU_ROSTER_V2_REFEREE="+json.dumps({
            ...
            if d.get("type")=="sonnet.roster.v1" and d.get("game_id")==GAME:
                print("MARU_ROSTER_V2_COUNTERSIGN="+json.dumps({
            ...
            if d.get("type")=="sonnet.receipt.v1" and d.get("roster_ready") is True:
                print("MARU_ROSTER_V2_ROSTER_READY_RECEIPT="+json.dumps({
...
if receipt is None:
    print("MARU_ROSTER_V2_WATCH=COMPLETE receipt_not_seen_yet")
else:
    print("MARU_ROSTER_V2_WATCH=COMPLETE")
PY
...
echo "MARU_ROSTER_V2_HELPER=COMPLETE"
