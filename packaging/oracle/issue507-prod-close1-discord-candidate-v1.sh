...
TARGET=5296f6dd74c54fc3bbee086ec99759af785037c5
RES_PID=2462149
CAP_PID=2462148
SIG_PID=2462068
DIS_PID=2560998
...
assert int(p.get("failure_count", 0) or 0) == 0
print(p["last_attempt_at"])
PY
}
...
PROGRESS_BEFORE=$(progress_gate) || stop_rollout close1_progress_state_not_healthy
...
EXPECTED='src/flop_agent/close1_candidate_scanner.py|src/flop_agent/close1_discord_progress.py|src/flop_agent/close1_strategy.py|src/flop_agent/close_call.py|tests/test_close1_candidate_scanner.py|tests/test_close1_discord_progress.py|tests/test_close1_strategy.py|tests/test_close_call.py|'
...
# Stability window only. Do not require a fresh progress-state write here:
# the worker intentionally throttles public polling to once per 5 minutes.
sleep 20
[[ "$(snap technocore-safe-agent-discord.service)" == "active|running|$NEW_DIS|0|success" ]] || stop_rollout discord_unstable_after_window
...
# Existing durable progress state must remain readable; its timestamp may
# legitimately be unchanged until the next 5-minute poll boundary.
PROGRESS_AFTER=$(progress_gate) || stop_rollout close1_progress_state_became_unhealthy
...
echo "PROGRESS_STATE=HEALTHY preserved_before:$PROGRESS_BEFORE after:$PROGRESS_AFTER"
