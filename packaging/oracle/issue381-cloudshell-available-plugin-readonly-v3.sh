...
if call_available "$TENANCY" "$ROOT_OUT" "$ROOT_ERR"; then
  ...
  ROOT_PARSE=$(parse_available_json "$ROOT_OUT")
  printf '%s\n' "$ROOT_PARSE"
  if printf '%s\n' "$ROOT_PARSE" | grep -qx 'PARSE_RESULT=PASS'; then
  ...
  echo 'TENANCY_SCOPE_PARSE=UNUSABLE'
  echo 'TENANCY_SCOPE_FALLBACK=ACCESSIBLE_COMPARTMENTS'
else
...
FINAL_PARSE=$(parse_available_json "$OUT")
printf '%s\n' "$FINAL_PARSE"
if ! printf '%s\n' "$FINAL_PARSE" | grep -qx 'PARSE_RESULT=PASS'; then
  echo 'CONTROL_PLANE_CALL=FAIL'
  echo 'CONTROL_PLANE_ERROR_CLASS=AVAILABLE_PLUGIN_PARSE'
  echo 'RAW_OCI_ERROR_OUTPUT=NO'
  echo 'MUTATION_COMMANDS=NONE'
  exit 0
fi
