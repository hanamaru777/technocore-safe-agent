...
UNIT=/run/systemd/system/technocore-safe-agent-sonnet-brucelead2-v2.service
UNIT_NAME=technocore-safe-agent-sonnet-brucelead2-v2.service
...
[[ "$("${GIT[@]}" rev-parse "$REF:src/flop_agent/sonnet_brucelead2_application_v2.py")" == "$V2_BLOB" ]] || fail v2_blob_mismatch
...
PY
) || fail prior_state_unreadable
[[ "$APP_STATE" == new ]] || fail "prior_state_not_new:$APP_STATE"
[[ "$REQUEST_ID" == maru-brucelead2-apply-20260915-1 ]] || fail prior_request_id_mismatch
[[ "$NONCE" == None && "$ATTEMPTED" == None && "$SEQ" == None && "$TS" == None && "$POSTED" == no ]] \
  || fail prior_state_not_presign

echo 'PRIOR_FAILED_ATTEMPT_STATE=VERIFIED_NEW_NO_NONCE_NO_POST'
...
echo "BRUCELEAD2_V2_PREFLIGHT=PASS health=$SAFETY_HEALTH age=$SAFETY_AGE core=$SAFETY_EVENTS/$SAFETY_MESSAGES"
...
"${GIT[@]}" show "$REF:src/flop_agent/sonnet_brucelead2_application_v2.py" > "$V2"
chmod 0444 "$LANE" "$V2"
cat > "$LAUNCHER" <<'PY'
...
v2 = load('flop_agent.sonnet_brucelead2_application_v2', '/run/sonnet_brucelead2_application_v2.py')
v2.main()
PY
chmod 0444 "$LAUNCHER"
...
cat > "$UNIT" <<'UNIT'
[Unit]
Description=Approved retained-export BRUCELEAD-2 non-binding application v2
...
ExecStart=/opt/technocore-safe-agent/.venv/bin/python /run/sonnet_brucelead2_v2_launcher.py
TimeoutStartSec=85s
...
ReadOnlyPaths=/run/sonnet_brucelead2_application.py /run/sonnet_brucelead2_application_v2.py /run/sonnet_brucelead2_v2_launcher.py
ReadWritePaths=/var/lib/technocore-safe-agent/signer /var/lib/technocore-safe-agent/nonces.json
UNIT

systemctl daemon-reload
systemctl reset-failed "$UNIT_NAME" >/dev/null 2>&1 || true
if ! timeout 95s systemctl start "$UNIT_NAME"; then
  echo 'BRUCELEAD2_V2=FAIL:oneshot'
