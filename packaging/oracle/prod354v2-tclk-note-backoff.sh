#!/usr/bin/env bash
# One-shot successor Production rollout for tclk Note retry classification/backoff.
# Fixes #355 helper-only false STOP caused by git show | grep -Fq under pipefail.
...
adapter_registry_empty() {
  sudo -u "$OWNER" python3 - "$APP/src/flop_agent/airdrop_executor.py" <<'PY'
from pathlib import Path
import sys

path=Path(sys.argv[1])
text=path.read_text("utf-8")
needle="_ADAPTERS: dict[str, ExecutionAdapter] = {}"
...
common_gate() {
  ...
  adapter_registry_empty || stop "$label:adapter_registry_not_empty"

  echo "PROD354V2_GATE=$label observer=$health core=$events/$messages requests=$requests candidates=$candidates executions=$executions"
}
...
rollback() {
  ...
  if [[ $DONE -eq 0 && $CUTOVER_STARTED -eq 1 ]]; then
    ...
    echo "PROD354V2_ROLLBACK=COMPLETE code=$PRE discord_reloaded=$DISCORD_RESTARTED" >&2
  fi
  exit "$rc"
}
trap rollback EXIT
...
[[ $ACTUAL == "$EXPECTED" ]] || {
  echo "PROD354V2=STOP:changed_file_allowlist_mismatch" >&2
  printf '%s\n' "$ACTUAL" >&2
  echo "DO_NOT_RERUN=YES" >&2
  exit 1
}

echo "PROD354V2_PREFLIGHT=PASS pre=$PRE target=$TARGET"

CUTOVER_STARTED=1
git_owner merge --ff-only "$TARGET" >/dev/null
[[ "$(git_owner rev-parse HEAD)" == "$TARGET" ]] || stop post_merge_head_mismatch

SMOKE_DIR=$(mktemp -d /tmp/prod354v2-tclk-note.XXXXXX)
...
printf '%s\n'   "PROD354V2=PASS"   "PRE_SHA=$PRE"   "POST_SHA=$(git_owner rev-parse HEAD)"   "PROTECTED_CORE=$FINAL_EVENTS/$FINAL_MESSAGES OBSERVER_HEALTH=$FINAL_HEALTH"   "ACTION_REQUESTS=$FINAL_REQUESTS CANDIDATES=$FINAL_CANDIDATES EXECUTIONS=$FINAL_EXECUTIONS"   "ADAPTER_REGISTRY=EMPTY"   "AIRDROP_TIMERS=$(systemctl is-active "$MON_TIMER")/$(systemctl is-active "$NOT_TIMER")"   "RESIDENT_PRESERVED=${PID_PRE[$RES]}/NRestarts=${RESTART_PRE[$RES]}"   "CAPTURE_PRESERVED=${PID_PRE[$CAP]}/NRestarts=${RESTART_PRE[$CAP]}"   "SIGNER_PRESERVED=${PID_PRE[$SIG]}/NRestarts=${RESTART_PRE[$SIG]}"   "DISCORD_RESTARTED=${PID_PRE[$DIS]}->$DIS_POST NRestarts=${RESTART_PRE[$DIS]}"   "ISOLATED_TCLK_NOTE_SMOKE=PASS"   "REGISTRY_CHECK=PYTHON_DIRECT_READ"   "SYNTHETIC_TECHNOCORE_ACTIVITY=NO HELPER_DISCORD_MESSAGE=NO"   "TCLK_ACCEPT=NO TCLK_REVEAL=NO TCLK_VALUE=NO SIGNER_CALL=NO FLOP_EXTERNAL_WRITE=NO X_WRITE=NO"   "DO_NOT_RERUN=YES"
