...
echo '=== ISSUE364 PRESSURE HEARTBEAT ROLLOUT V1 ==='
echo "UTC=$(date -u '+%Y-%m-%dT%H:%M:%SZ')"

stop_pre() {
  echo "PROD364HBV1=STOP:$1"
  exit 0
}

if [[ $EUID -ne 0 ]]; then stop_pre not_root; fi
for cmd in git systemctl stat runuser timeout sleep cut seq; do
...
PRE_STATE=$(FLOP_STATE_DIR="$STATE" PYTHONPATH="$APP/src" "$APP_PY" - "$OBS" "$OBHB" "$RESHB" <<'PY'
...
raw=(reshb.get("resident_status") or {}).get("last_refresh_at","missing")
print(f"{int(m.get('unrecoverable_core_gap_events',0) or 0)}|{int(m.get('unrecoverable_core_gap_messages',0) or 0)}|{int(c.get('lobby',0) or 0)}|{obs.get('updated_at','missing')}|{age(obhb.get('updated_at')):.1f}|{age(reshb.get('updated_at')):.1f}|{reshb.get('status','missing')}|{raw}")
PY
)
...
PRE_TRUE_REFRESH=$(printf '%s' "$PRE_STATE"|cut -d'|' -f8)
...
echo "PRE_TRUE_MAINTENANCE_REFRESH=$PRE_TRUE_REFRESH"
...
check_file "$RESIDENT_FILE" "$PRE_RESIDENT_BLOB" || stop_pre resident_file_changed
...
git -C "$APP" merge --ff-only "$TARGET" || { echo 'PROD364HBV1=STOP:ff_only_failed'; exit 0; }
POST_HEAD=$(git -C "$APP" rev-parse HEAD 2>/dev/null || true)
echo "POST_HEAD=$POST_HEAD"
[[ "$POST_HEAD" == "$TARGET" ]] || { echo 'PROD364HBV1=STOP:post_head_wrong'; echo 'TARGET_MAY_BE_PARTIALLY_APPLIED=YES'; exit 0; }

# Keep canonical Python source permissions explicit after the prior Production
...
check_file "$CAPTURE_FILE" "$CAPTURE_BLOB" || { echo 'PROD364HBV1=STOP:capture_file_post_invalid'; exit 0; }
check_file "$BRIDGE_FILE" "$BRIDGE_BLOB" || { echo 'PROD364HBV1=STOP:bridge_file_post_invalid'; exit 0; }
check_file "$ISOLATION_FILE" "$TARGET_ISOLATION_BLOB" || { echo 'PROD364HBV1=STOP:isolation_file_post_invalid'; exit 0; }
check_file "$RESIDENT_FILE" "$TARGET_RESIDENT_BLOB" || { echo 'PROD364HBV1=STOP:resident_file_post_invalid'; exit 0; }
...
[[ -z "$POST_WORKTREE" ]] || { echo 'PROD364HBV1=STOP:worktree_dirty_after_deploy'; exit 0; }
...
from flop_agent import observer_resident_isolation, resident
import flop_agent.resident_daemon
assert resident.PRESSURE_PAUSED_HEARTBEAT_STATUS == "pressure_paused"
assert callable(resident.write_pressure_heartbeat)
assert callable(observer_resident_isolation._write_pressure_heartbeat)
print("IMPORT_SMOKE=PASS")
PY
then
  echo 'PROD364HBV1=STOP:import_smoke_failed'
  exit 0
fi
...
[[ "$RESTART_RC" == 0 ]] || { echo 'PROD364HBV1=STOP:resident_restart_command_failed'; echo 'TARGET_REMAINS_DEPLOYED=YES'; exit 0; }
...
[[ -n "$NEW_PID" ]] || { echo 'PROD364HBV1=STOP:resident_not_recovered'; echo 'TARGET_REMAINS_DEPLOYED=YES'; exit 0; }
...
raw_status=reshb.get("resident_status") if isinstance(reshb.get("resident_status"),dict) else {}
...
print(f"{label}_RESIDENT_HEARTBEAT_TOP_STATUS={reshb.get('status','missing')}")
print(f"{label}_RESIDENT_HEARTBEAT_AGE={age(reshb.get('updated_at')):.1f}")
print(f"{label}_RAW_MAINTENANCE_LAST_REFRESH={raw_status.get('last_refresh_at','missing')}")
print(f"{label}_OPERATIONAL_LAST_REFRESH={op.get('last_refresh_at','missing')}")
print(f"{label}_OPERATIONAL_MAINTENANCE_STATUS={op.get('maintenance_status','missing')}")
print(f"{label}_OPERATIONAL_MAINTENANCE_LAST_REFRESH={op.get('maintenance_last_refresh_at','missing')}")
semantics=True
if reshb.get("status") == resident.PRESSURE_PAUSED_HEARTBEAT_STATUS:
    semantics=(
        ...
        and op.get("maintenance_last_refresh_at") == raw_status.get("last_refresh_at")
...
if pid and children.exists():
    for raw in children.read_text("utf-8").split():
        try:child=int(raw)
...
if [[ "$FINAL_ACTIVE" != active || "$FINAL_SUB" != running || "$FINAL_PID" != "$NEW_PID" || "$FINAL_NR" != 0 || "$FINAL_RESULT" != success ]]; then
  echo 'PROD364HBV1=STOP:resident_unstable'
  echo 'TARGET_REMAINS_DEPLOYED=YES'
  exit 0
fi

FINAL=$(FLOP_STATE_DIR="$STATE" PYTHONPATH="$APP/src" "$APP_PY" - "$OBS" "$OBHB" "$RESHB" "$PRE_CURSOR" "$PRE_OBS_UPDATED" <<'PY'
import json,pathlib,sys
from datetime import UTC,datetime
from flop_agent import resident
obs=json.loads(pathlib.Path(sys.argv[1]).read_text("utf-8"))
...
F_RES_HB_STATUS=$(printf '%s' "$FINAL"|cut -d'|' -f7)
F_SEMANTICS=$(printf '%s' "$FINAL"|cut -d'|' -f8)
F_UPDATED_MOVED=$(printf '%s' "$FINAL"|cut -d'|' -f9)
F_CURSOR_OK=$(printf '%s' "$FINAL"|cut -d'|' -f10)
...
echo "FINAL_RESIDENT_HEARTBEAT_STATUS=$F_RES_HB_STATUS"
echo "FINAL_RESIDENT_HEARTBEAT_SEMANTICS=$F_SEMANTICS"
echo "FINAL_OBSERVER_UPDATED_MOVED=$F_UPDATED_MOVED"
echo "FINAL_CURSOR_NONDECREASING=$F_CURSOR_OK"

[[ "$F_EVENTS" == "$EXPECTED_CORE_EVENTS" && "$F_MESSAGES" == "$EXPECTED_CORE_MESSAGES" ]] || { echo 'PROD364HBV1=STOP:protected_core_changed'; echo 'TARGET_REMAINS_DEPLOYED=YES'; exit 0; }
[[ "$F_SEMANTICS" == 1 ]] || { echo 'PROD364HBV1=STOP:heartbeat_semantics_invalid'; echo 'TARGET_REMAINS_DEPLOYED=YES'; exit 0; }
[[ "$F_UPDATED_MOVED" == 1 && "$F_CURSOR_OK" == 1 ]] || { echo 'PROD364HBV1=STOP:observer_not_progressing'; echo 'TARGET_REMAINS_DEPLOYED=YES'; exit 0; }
...
PY
then
  echo 'PROD364HBV1=STOP:heartbeat_not_fresh'
  echo 'TARGET_REMAINS_DEPLOYED=YES'
  exit 0
fi

service_exact CAPTURE "$CAP" "$EXPECTED_CAP_PID" "$EXPECTED_CAP_RESTARTS" || { echo 'PROD364HBV1=STOP:capture_changed'; exit 0; }
service_exact SIGNER "$SIGN" "$EXPECTED_SIGN_PID" "$EXPECTED_SIGN_RESTARTS" || { echo 'PROD364HBV1=STOP:signer_changed'; exit 0; }
service_exact DISCORD "$DISC" "$EXPECTED_DISC_PID" "$EXPECTED_DISC_RESTARTS" || { echo 'PROD364HBV1=STOP:discord_changed'; exit 0; }
META_ACTIVE=$(systemctl is-active "$META" 2>/dev/null || true)
META_RESULT=$(systemctl show "$META" -p Result --value 2>/dev/null || true)
echo "FINAL_METADATA_BLOCK ACTIVE=$META_ACTIVE RESULT=$META_RESULT"
[[ "$META_ACTIVE" == active && "$META_RESULT" == success ]] || { echo 'PROD364HBV1=STOP:metadata_block_changed'; exit 0; }
...
echo '=== PROD364_PRESSURE_HEARTBEAT_ROLLOUT_V1=PASS ==='
