...
fail() {
  trap - ERR
  echo "SONNET_OPEN_SEAT_BROADCAST=STOP:$1"
...
REF=refs/remotes/origin/sonnet-open-seat-broadcast-helper
PROD_HEAD=baf5a3e1d5d36a24b28cf9fa7dfa2dd41ccdf20f
MODULE_BLOB=117d859aff17590ab80efe252f7d2ebe658fe686
MODULE=/run/sonnet_open_seat_broadcast_20260916.py
LAUNCHER=/run/sonnet_open_seat_broadcast_20260916_launcher.py
UNIT=/run/systemd/system/technocore-safe-agent-sonnet-open-seat-broadcast.service
UNIT_NAME=technocore-safe-agent-sonnet-open-seat-broadcast.service
STATE=/var/lib/technocore-safe-agent/signer/sonnet-2-open-seat-broadcast-20260916.json
...
[[ "$("${GIT[@]}" rev-parse "$REF:src/flop_agent/sonnet_open_seat_broadcast_20260916.py")" == "$MODULE_BLOB" ]] || fail module_blob_mismatch
...
echo "SONNET_OPEN_SEAT_PREFLIGHT=PASS health=$HEALTH age=$AGE core=$EVENTS/$MESSAGES"
...
"${GIT[@]}" show "$REF:src/flop_agent/sonnet_open_seat_broadcast_20260916.py" > "$MODULE"
chmod 0444 "$MODULE"
cat > "$LAUNCHER" <<'PY'
import importlib.util
import sys
name = "flop_agent.sonnet_open_seat_broadcast_20260916"
spec = importlib.util.spec_from_file_location(name, "/run/sonnet_open_seat_broadcast_20260916.py")
...
cat > "$UNIT" <<'UNIT'
[Unit]
Description=Fixed non-binding MARU Sonnet-2 open-seat broadcast
...
ExecStart=/opt/technocore-safe-agent/.venv/bin/python /run/sonnet_open_seat_broadcast_20260916_launcher.py
...
ReadOnlyPaths=/run/sonnet_open_seat_broadcast_20260916.py /run/sonnet_open_seat_broadcast_20260916_launcher.py
...
if ! timeout 190s systemctl start "$UNIT_NAME"; then
  echo 'SONNET_OPEN_SEAT_BROADCAST=FAIL:oneshot'
  if [[ -f "$STATE" ]]; then
    timeout 5s python3 - "$STATE" <<'PY' || true
import json, sys
try:
    with open(sys.argv[1], encoding="utf-8") as f: d=json.load(f)
    print(f"FAIL_STATE={d.get('state')} ATTEMPTED={'YES' if d.get('attempted_at') else 'NO'} SEQ={d.get('seq')} TS={d.get('ts')}")
...
assert d.get("request_id") == "maru-open-seat-broadcast-20260916-1"
assert d.get("state") == "posted"
assert isinstance(d.get("seq"), int) and d["seq"] >= 0
assert isinstance(d.get("ts"), str) and d["ts"]
print(f"BROADCAST_STATE=posted REQUEST_ID={d.get('request_id')} SEQ={d.get('seq')} TS={d.get('ts')}")
PY
...
trap - ERR
echo 'SONNET_OPEN_SEAT_BROADCAST=PASS'
