#!/usr/bin/env bash
# PROD376 V2: persistent least-privilege OCA snap_daemon IMDS repair.
...
rollback_and_cleanup() {
  ...
  if [[ $MUTATED -eq 1 && $DONE -eq 0 ]]; then
    ...
    echo "PROD376V2_ROLLBACK_FILE_RC=$file_rc"
    echo "PROD376V2_ROLLBACK_FIREWALL_RC=$fw_rc"
    echo 'PROD376V2_ROLLBACK=COMPLETE'
  fi
  rm -rf -- "$TMPDIR"
  exit "$rc"
}
...
echo '=== PROD376 V2 OCA SNAP_DAEMON IMDS REPAIR ==='
