...
MODULE_BLOB=532d7a14bea194a2110c86ac2df89e085f84a303
MODULE=/run/sonnet_opportunity_sweep_20260916_v2.py
LAUNCHER=/run/sonnet_opportunity_sweep_20260916_v2_launcher.py
UNIT=/run/systemd/system/technocore-safe-agent-sonnet-opportunity-sweep-v2.service
UNIT_NAME=technocore-safe-agent-sonnet-opportunity-sweep-v2.service
...
[[ "$("${GIT[@]}" rev-parse "$REF:src/flop_agent/sonnet_opportunity_sweep_20260916_v2.py")" == "$MODULE_BLOB" ]] || fail module_blob_mismatch
...
"${GIT[@]}" show "$REF:src/flop_agent/sonnet_opportunity_sweep_20260916_v2.py" > "$MODULE"
chmod 0444 "$MODULE"
cat > "$LAUNCHER" <<'PY'
import importlib.util
import sys
name = "flop_agent.sonnet_opportunity_sweep_20260916_v2"
spec = importlib.util.spec_from_file_location(name, "/run/sonnet_opportunity_sweep_20260916_v2.py")
...
cat > "$UNIT" <<'UNIT'
[Unit]
Description=Fixed non-binding Sonnet-2 opportunity sweep v2 (magnatsv + luxion status)
...
ExecStart=/opt/technocore-safe-agent/.venv/bin/python /run/sonnet_opportunity_sweep_20260916_v2_launcher.py
...
ReadOnlyPaths=/run/sonnet_opportunity_sweep_20260916_v2.py /run/sonnet_opportunity_sweep_20260916_v2_launcher.py
