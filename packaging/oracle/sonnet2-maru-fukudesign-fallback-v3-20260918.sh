...
REQ=maru-fukudesign-fallback-inquiry-20260918-3
...
python3 - "$AGE" <<'PY' || stop "observer_state_stale:$AGE"
import sys
a=float(sys.argv[1]); raise SystemExit(0 if 0 <= a <= 120 else 1)
PY
echo "MARU_FUKU_FALLBACK_PREFLIGHT=PASS source=observer-state health=$HEALTH age=$AGE core=$EVENTS/$MESSAGES"
...
RID="maru-fukudesign-fallback-inquiry-20260918-3"
