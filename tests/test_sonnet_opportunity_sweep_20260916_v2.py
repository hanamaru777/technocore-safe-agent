...
from flop_agent import sonnet_opportunity_sweep_20260916_v2 as sweep
...
def test_payloads_are_fixed_nonbinding_and_luxion_payload_has_no_freeze_keyword():
    ...
    assert "yes-luxion" not in luxion["text"].lower()
    assert "NOT a freeze response" in luxion["text"]
    assert "Do not freeze or roster MARU" in luxion["text"]
    for payload in (magnatsv, luxion):
        assert payload["did"] == sweep.DID
        assert payload["role"] == "writer"
        assert sweep.REGISTRATION_REQUEST in payload["text"]
...
def test_post_one_marks_attempt_before_single_post(monkeypatch, state_root):
    ...
    class Response:
        ...
        def json(self):
            return {"posted": {"from": sweep.DID, "nonce": "12345", "text": text, "sig": "sig", "seq": 111111, "ts": "2026-09-16T00:00:00Z"}}
...
def test_post_transport_failure_is_terminal_ambiguous(monkeypatch, state_root):
    ...
    with pytest.raises(sweep.SweepError, match="magnatsv_submission_ambiguous"):
        sweep._post_one(sweep.load(), fixed)


def test_module_has_no_busy_room_read_or_export():
    source = Path("src/flop_agent/sonnet_opportunity_sweep_20260916_v2.py").read_text("utf-8")
