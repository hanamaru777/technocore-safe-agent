...
def test_blocking_error_has_exact_token_and_missing_letter_diagnostics():
    roster = [LIMITED, "did:key:z6Mk" + "B" * 44, "did:key:z6Mk" + "C" * 44, "did:key:z6Mk" + "D" * 44]
    try:
...
def test_cli_success_emits_json_without_network(monkeypatch, tmp_path, capsys):
    ...
    code = tool.main(["--roster", str(roster), "--poem", str(poem), "--dictionary", str(tmp_path / "cmudict.dict")])
...
def test_tool_has_no_network_shell_signer_or_secret_surface():
    ...
    forbidden = ("httpx", "requests", "urllib", "socket", "subprocess", "os.system", "post_signed", "invoke_signer", "SIGN_SEED", "with_vault_seed")
