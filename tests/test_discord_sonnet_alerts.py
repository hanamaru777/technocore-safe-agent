...
def signed_invite(seq, request_id="maru-invite-lon-20260914-1", *, ts="1970-01-01T00:00:01Z"):
    return {
        "from": alerts.INVITED_DIDS[request_id], "nonce": str(seq), "sig": "A" * 86,
        "seq": seq, "ts": ts,
        "text": ('{"type":"sonnet.invite-response.v1","contest_id":"sonnet-2",' f'"request_id":"{request_id}"' "}"),
    }
...
def test_multiple_new_github_rows_in_one_poll_are_all_notified(monkeypatch, tmp_path):
    ...
    alerts._save(state)
    newer_a = row(12, "MARU writer disposition", created="1970-01-01T00:20:00Z")
    newer_b = row(11, "MARU writer disposition", created="1970-01-01T00:19:00Z")
...
def test_source_that_failed_first_poll_baselines_on_its_first_success(monkeypatch, tmp_path):
    ...
    first = {"value": True}
    def fetch(issue):
        if issue == 16 and first["value"]: raise httpx.TimeoutException("x")
        if issue == 16: return [historical]
        return []
...
def test_room_evidence_is_signed_and_never_calls_reply_consent(monkeypatch):
    ...
    team = {"from": alerts.REFEREE_DID, "nonce": "2", "sig": "A" * 86, "seq": 2, "ts": "1970-01-01T00:00:02Z", "text": '{"contest_id":"sonnet-2","game_id":"maru73s2","poem_room":"d-sonnet-2-team-maru73s2","type":"sonnet.setup.v1","room_generation":1}'}
...
def test_room_cursor_uses_since_and_failed_initial_room_does_not_replay(monkeypatch, tmp_path):
    configure(monkeypatch, tmp_path)
    monkeypatch.setattr(alerts, "verify_signed_record", lambda *_a: None)
    calls = []; fail_discovery = {"value": True}
    historical = signed_invite(10, ts="1970-01-01T00:00:01Z"); newer = signed_invite(11, ts="1970-01-01T00:30:00Z")
    def room_read(room, since=None):
        calls.append((room, since))
        if room == alerts.DISCOVERY_ROOM and fail_discovery["value"]: raise httpx.TimeoutException("x")
        if room == alerts.DISCOVERY_ROOM: return [historical] if since is None else ([newer] if since == 10 else [])
        return []
    ...
    assert (alerts.DISCOVERY_ROOM, None) in calls and (alerts.DISCOVERY_ROOM, 10) in calls
...
def test_unrecoverable_room_gap_emits_one_fail_closed_notice(monkeypatch, tmp_path):
    configure(monkeypatch, tmp_path)
    state = alerts._default(); state["rooms"][alerts.DISCOVERY_ROOM] = {"initialized": True, "seq": 1}; alerts._save(state)
    retained = [signed_invite(3), signed_invite(4)]
    def room_read(room, since=None):
        if room == alerts.DISCOVERY_ROOM: raise alerts.RoomRetentionGap(room, 2, 2, retained)
        return []
    monkeypatch.setattr(alerts, "verify_signed_record", lambda *_a: None)
    ...
    assert alerts._load()["rooms"][alerts.DISCOVERY_ROOM]["seq"] == 4
...
def test_more_than_four_notices_are_delivered_from_pending_without_loss(monkeypatch, tmp_path):
    configure(monkeypatch, tmp_path)
    state = alerts._default(); state["github"]["gh:25"] = {"initialized": True, "max_id": 0, "max_updated_at": ""}
    for issue in (16, 22): state["github"][f"gh:{issue}"] = {"initialized": True, "max_id": 0, "max_updated_at": ""}
    state["rooms"][alerts.DISCOVERY_ROOM] = {"initialized": True, "seq": 0}; state["rooms"][alerts.RESULTS_ROOM] = {"initialized": True, "seq": 0}; alerts._save(state)
