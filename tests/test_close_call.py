...
def test_verified_public_offer_rejects_unknown_metadata_even_in_actual_room():
    record = signed_public_offer(room="close1-offers", include_how=True)
    ...
    maker_key = Ed25519PrivateKey.from_private_bytes(b"\x11" * 32)
    record["text"] = text
    record["sig"] = _sig(maker_key, f"close1-offers|{record['nonce']}|{text}")
    with pytest.raises(ValueError, match="offer_shape_invalid"):
        close_call.parse_verified_public_offer(
            record, current_sweep=189, our_did=DID_B, room="close1-offers"
        )
