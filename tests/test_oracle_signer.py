...
def test_oracle_signer_never_reposts_prepared_receipt(monkeypatch, tmp_path):
    _, item = setup(monkeypatch, tmp_path); text = autopilot.render(item)
    receipt = {"state": "prepared", "did": SIGNER_DID, "nonce": "123", "text_hash": hashlib.sha256(text.encode()).hexdigest(), "receipt_hash": oracle_signer.receipt_hash(autopilot.export_intent(item), SIGNER_DID, "123", text), "prepared_at": datetime.now(UTC).isoformat()}
    ...
    monkeypatch.setattr(core, "post_signed", lambda *args, **kwargs: pytest.fail("prepared receipt must never repost"))
    assert oracle_signer.run_once()["processed"][0]["action"] == "pending_reconciliation"
