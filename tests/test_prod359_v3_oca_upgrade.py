...
HELPER = core.ROOT / "packaging" / "oracle" / "prod359-v3-oca-ubuntu-runcommand-upgrade.sh"
...
def test_prod359_v3_helper_bash_syntax() -> None:
    subprocess.run(["bash", "-n", str(HELPER)], check=True)


def test_prod359_v3_helper_pins_reconciled_runtime_state() -> None:
    text = _text()
...
def test_prod359_v3_helper_has_exactly_one_refresh_and_one_revert() -> None:
    text = _text()
...
def test_prod359_v3_helper_eliminates_pipefail_early_consumer_parsers() -> None:
    text = _text()
...
def test_prod359_v3_helper_uses_evidence_based_continuity_gate() -> None:
    text = _text()
    ...
    for token in (
        ...
        "capture_no_activity",
        "EVIDENCE_BASED_CONTINUITY_GATE=PASS",
        "ACTIVE_CAPTURE_SQLITE_QUERY=NO",
    ):
        assert token in text

    assert '[[ $health == ok || $health == degraded ]]' in text
    assert '[[ $observer_status == ok || $observer_status == degraded ]]' in text
    assert "observer_not_ready" not in text


def test_prod359_v3_helper_direct_probes_are_read_only_and_same_user() -> None:
    text = _text()
    ...
    assert '"format":"json","since":0,"wait":0,"limit":1' in text
...
def test_prod359_v3_helper_capture_liveness_avoids_active_sqlite_query() -> None:
    text = _text()
...
def test_prod359_v3_helper_revalidates_immediately_around_mutation() -> None:
    text = _text()
...
def test_prod359_v3_helper_preserves_application_processes() -> None:
    text = _text()
    lowered = text.lower()
...
def test_prod359_v3_helper_preserves_snap_tracking_hold_and_security_boundary() -> None:
    text = _text()
    lowered = text.lower()
...
def test_prod359_v3_helper_has_deterministic_revert_after_applied_refresh() -> None:
    text = _text()

    assert "REFRESH_COMPLETED=0" in text
    assert "REFRESH_COMPLETED=1" in text
    assert 'if [[ $DONE -eq 0 && $REFRESH_COMPLETED -eq 1 ]]' in text
    assert "PROD359V3_ROLLBACK_RC=" in text
    assert "PROD359V3_ROLLBACK_VERSION=" in text
    assert "PROD359V3_ROLLBACK_REVISION=" in text
    assert "DO_NOT_RERUN=YES" in text


def test_prod359_v3_helper_observes_runcommand_surface_without_creating_one() -> None:
    text = _text()
    ...
    lowered = text.lower()
    assert "run command create" not in lowered
    assert "instance-agent command create" not in lowered


def test_prod359_v3_helper_has_no_application_or_external_writes() -> None:
    text = _text()
    lowered = text.lower()
