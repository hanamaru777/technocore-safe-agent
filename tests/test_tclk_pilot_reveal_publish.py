...
def reveal_preview():
    return {
        ...
        "accept_sha256": "7" * 64,
        "lock_line_sha256": "8" * 64,
        "lock_ref": CONTRACT,
        "paper_note_sha256": "9" * 64,
        "work_evidence_sha256": "a" * 64,
        "expires_ms": 2_000_001_000_000,
        "claim_by_ms": 2_000_000_900_000,
        "refund_after_ms": 2_000_001_200_000,
        "reveal_sha256": "b" * 64,
    }
...
def test_reveal_approval_is_distinct_and_public_only(monkeypatch):
    ...
    changed["work_evidence_sha256"] = "c" * 64
...
def test_ambiguous_reveal_never_reposts(monkeypatch):
    ...
    result = publish.publish_stage(STAGE, now_ms=2_000_000_000_000)
    assert result["action"] == "reveal_ambiguous"
...
def test_write_interlock_runs_before_any_new_source_or_sign(monkeypatch):
    ...
    monkeypatch.setattr(
        publish.tclk_pilot_accept,
        "_require_write_interlock",
        lambda: (_ for _ in ()).throw(publish.tclk_pilot_accept.AcceptError("observer_health_not_ok")),
    )
...
const now=Date.now(); const secret='0x'+'11'.repeat(32);
...
const accept=makeAccept(offer,{from:i.payee,statement:hashLockFromPreimage(secret).hash,nonce:'12345678'});
const reveal='tclk1 '+JSON.stringify({contract:accept.contract,from:i.payee,secret,type:'reveal'},Object.keys({contract:0,from:0,secret:0,type:0}).sort());
const locked=encodePaperRecord({status:'locked',lock:'hash',statement:accept.statement,refundAfterMs:offer.refundAfterMs});
process.stdout.write(JSON.stringify({accept:encodeFrame(accept),reveal,contract:accept.contract,locked,refund:offer.refundAfterMs,now,secret}));
"""
    ...
    assert v["secret"] in out["claimed_line"]
...
def test_reveal_wrapper_and_unit_are_fixed_shape_without_timer():
    ...
    assert "APPROVE_REVEAL" in wrapper
    assert "technocore-tclk-approve" not in wrapper
...
def test_public_result_never_exposes_reveal_line_or_secret():
    state = {
        "stage_id": STAGE,
        "contract_id": CONTRACT,
        "deal_room": "mb-p-tclk-" + "6" * 16,
        "reveal_sha256": "a" * 64,
        "state": "claimed",
        "claimed_note_sha256": "b" * 64,
        "reveal_line": "must-not-leak",
        "preimage": "must-not-leak",
    }
    result = publish.public_result({"action": "claimed", "state": state})
