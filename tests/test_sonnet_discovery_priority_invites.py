...
from flop_agent import sonnet_discovery_invites as old_lane
from flop_agent import sonnet_discovery_murphy_invite as murphy_lane
from flop_agent import sonnet_discovery_priority_invites as lane
...
def environment(monkeypatch, tmp_path):
    ...
    class Clock(datetime):
        @classmethod
        def now(cls, tz=None):
            return cls(2026, 9, 14, 9, tzinfo=UTC)
...
def test_exact_two_fixed_priority_invitations_post_once(environment):
    _, posts, signs, buffers, _ = environment
    assert [item["request_id"] for item in lane.INVITATIONS] == [
        "maru-invite-lon-20260914-1",
        "maru-invite-fuego-20260914-1",
    ]
    assert [item["target_did"] for item in lane.INVITATIONS] == [
        "did:key:z6MkqRmKyNcvzfYX2PRXj2TRoXMYDKDy6RQzL6VAwj4DBhhj",
        "did:key:z6Mkt6jAezZ7WyPyN1bTXX63ps4vn8MSvNwtfA9JFgsqeFTB",
    ]
    result = lane.run_once()
    assert result["status"] == "complete"
    assert len(posts) == len(signs) == 2
...
def test_wrong_room_and_cli_args_rejected(monkeypatch):
    ...
    monkeypatch.setattr(sys, "argv", ["sonnet_discovery_priority_invites", "extra"])
...
@pytest.mark.parametrize("failed_post", [1, 2])
def test_post_ambiguity_is_terminal_and_later_targets_not_blindly_sent(
    environment, monkeypatch, failed_post
):
    _, posts, signs, _, _ = environment
    ...
    def fail_selected(*args, **kwargs):
        count["value"] += 1
        if count["value"] == failed_post:
            posts.append(kwargs["json"])
            raise core.httpx.ConnectTimeout("dummy")
        return original(*args, **kwargs)

    monkeypatch.setattr(core.httpx, "post", fail_selected)
    with pytest.raises(lane.InviteError, match="submission_unknown"):
        lane.run_once()
    state = lane.load()
    ids = [item["request_id"] for item in lane.INVITATIONS]
    assert state["entries"][ids[failed_post - 1]]["state"] == "ambiguous"
    if failed_post == 1:
        assert state["entries"][ids[1]]["state"] == "new"
    else:
...
def test_error_output_never_leaks_exception(monkeypatch, capsys):
    monkeypatch.setattr(sys, "argv", ["sonnet_discovery_priority_invites"])
    monkeypatch.setattr(
        lane,
        "run_once",
...
def test_new_lane_is_state_isolated_from_completed_and_murphy_lanes(monkeypatch, tmp_path):
    monkeypatch.setattr(core, "STATE", tmp_path)
    assert lane.state_path().name == "sonnet-2-discovery-priority-invites.json"
    assert lane.state_path() != old_lane.state_path()
    assert lane.state_path() != murphy_lane.state_path()
    old_ids = {item["request_id"] for item in old_lane.INVITATIONS}
    new_ids = {item["request_id"] for item in lane.INVITATIONS}
    assert old_ids.isdisjoint(new_ids)
    assert murphy_lane.REQUEST_ID not in new_ids
    assert len(old_lane.INVITATIONS) == 3


def test_one_shot_unit_preserves_isolation_and_cannot_auto_start():
    text = (
        core.ROOT
        / "packaging/oracle/technocore-safe-agent-sonnet-priority-invites.service"
    ).read_text("utf-8")
    for required in (
        "Type=oneshot",
        ...
        "-m flop_agent.sonnet_discovery_priority_invites",
    ):
