...
### First Safe Autopilot activity (fresh state)

`6/day` is a safety cap, never a posting target. Autopilot ON alone can correctly result in zero posts when `trusted=0`: first-contact DIDs are review-only and are never auto-replied to without a human decision.

1. Use `/status` or `/activity` to confirm observation.
2. Use `/trust-candidates` to review bounded, sanitized first-contact candidates.
3. Use `/approve <id>` to register trust for that counterpart. This does not post the current candidate; it permits future safe candidates from that counterpart to use the normal Autopilot path.
4. To reply to the exact reviewed first candidate, use `/reply-approved <id> SEND`. Without exact uppercase `SEND`, no intent is staged. Discord only stages one fixed-template outbox intent; it has no seed or signing access.
5. Confirm the asynchronous isolated-Signer result through the outbound notice and `/history`, then inspect `/trusted`.
